 ...
 *          DO NOT MODIFY THIS FILE, MODIFY src/sans/models/include/pringles.h
...
static PyObject * CPringlesModelError = NULL;
...
static void
CPringlesModel_dealloc(CPringlesModel* self)
{
...
static PyObject *
CPringlesModel_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
    CPringlesModel *self;
    
    self = (CPringlesModel *)type->tp_alloc(type, 0);
   
    return (PyObject *)self;
}

static int
CPringlesModel_init(CPringlesModel *self, PyObject *args, PyObject *kwds)
{
    if (self != NULL) {
	...
	self->model = new PringlesModel();
        ...
        PyDict_SetItemString(self->params,"sld_solvent",Py_BuildValue("d",0.000006350000));
        PyDict_SetItemString(self->params,"thickness",Py_BuildValue("d",10.000000000000));
        PyDict_SetItemString(self->params,"beta",Py_BuildValue("d",0.020000000000));
        PyDict_SetItemString(self->params,"radius",Py_BuildValue("d",60.000000000000));
        PyDict_SetItemString(self->params,"sld_pringle",Py_BuildValue("d",0.000001000000));
        PyDict_SetItemString(self->params,"alpha",Py_BuildValue("d",0.001000000000));
        ...
        disp_dict = PyDict_New();
        self->model->alpha.dispersion->accept_as_source(visitor, self->model->alpha.dispersion, disp_dict);
        PyDict_SetItemString(self->dispersion, "alpha", disp_dict);
        disp_dict = PyDict_New();
        self->model->beta.dispersion->accept_as_source(visitor, self->model->beta.dispersion, disp_dict);
        PyDict_SetItemString(self->dispersion, "beta", disp_dict);
...
static PyMemberDef CPringlesModel_members[] = {
    {name_params, T_OBJECT, offsetof(CPringlesModel, params), 0, def_params},
	{name_dispersion, T_OBJECT, offsetof(CPringlesModel, dispersion), 0, def_dispersion},     
    {name_log, T_OBJECT, offsetof(CPringlesModel, log), 0, def_log},
    {NULL}  /* Sentinel */
};
...
/** Read double from PyObject
    ...
    @return double
*/
double CPringlesModel_readDouble(PyObject *p) {
...
static PyObject *evaluateOneDim(PringlesModel* model, PyArrayObject *q){
 ...
 * @return: numpy array object 
 */
 static PyObject * evaluateTwoDimXY( PringlesModel* model, 
...
#pragma omp parallel for
        ...
        return PyArray_Return(result); 
		    ...
		    PyErr_SetString(CPringlesModelError, 
                   "CPringlesModel.evaluateTwoDimXY couldn't run.");
	        return NULL;
		}      	
}
/**
 ...
 *
 */ 
static PyObject * evalDistribution(CPringlesModel *self, PyObject *args){
	PyObject *qx, *qy;
	PyArrayObject * pars;
	int npars ,mpars;
    ...
    self->model->beta = PyFloat_AsDouble( PyDict_GetItemString(self->params, "beta") );
    self->model->radius = PyFloat_AsDouble( PyDict_GetItemString(self->params, "radius") );
    self->model->sld_pringle = PyFloat_AsDouble( PyDict_GetItemString(self->params, "sld_pringle") );
    self->model->alpha = PyFloat_AsDouble( PyDict_GetItemString(self->params, "alpha") );
    ...
    disp_dict = PyDict_GetItemString(self->dispersion, "alpha");
    self->model->alpha.dispersion->accept_as_destination(visitor, self->model->alpha.dispersion, disp_dict);
    disp_dict = PyDict_GetItemString(self->dispersion, "beta");
    self->model->beta.dispersion->accept_as_destination(visitor, self->model->beta.dispersion, disp_dict);

	
	// Get input and determine whether we have to supply a 1D or 2D return value.
	if ( !PyArg_ParseTuple(args,"O",&pars) ) {
	    PyErr_SetString(CPringlesModelError, 
	    	"CPringlesModel.evalDistribution expects a q value.");
		return NULL;
	}
    // Check params
    ...
    if(PyArray_Check(pars)==1) {
	    ...
	    if(npars==1) {
		    ...
		    PyErr_SetString(CPringlesModelError, 
                   "CPringlesModel.evalDistribution expect numpy array of one dimension.");
	        return NULL;
		}
	    ...
	    if(mpars!=2) {
	    	PyErr_SetString(CPringlesModelError, 
	    		"CPringlesModel.evalDistribution expects a list of dimension 2.");
	    	return NULL;
	    }
	     ...
	     if (PyArray_Check(qx) && PyArray_Check(qy)) {
		    ...
		    PyErr_SetString(CPringlesModelError, 
                   "CPringlesModel.evalDistribution expect 2 numpy arrays in list.");
	        return NULL;
	     }
	}
	PyErr_SetString(CPringlesModelError, 
                   "CPringlesModel.evalDistribution couln't be run.");
	return NULL;
 ...
 */
static PyObject * run(CPringlesModel *self, PyObject *args) {
	...
	if ( !PyArg_ParseTuple(args,"O",&pars) ) {
	    PyErr_SetString(CPringlesModelError, 
	    	"CPringlesModel.run expects a q value.");
		return NULL;
	}
	...
	if( PyList_Check(pars)==1) {
	    ...
	    if(npars!=2) {
	    	PyErr_SetString(CPringlesModelError, 
	    		"CPringlesModel.run expects a double or a list of dimension 2.");
	    	return NULL;
	    }
	    // We have a vector q, get the q and phi values at which
	    // to evaluate I(q,phi)
	    q_value = CPringlesModel_readDouble(PyList_GET_ITEM(pars,0));
	    phi_value = CPringlesModel_readDouble(PyList_GET_ITEM(pars,1));
	    // Skip zero
	    if (q_value==0) {
		...
		q_value = CPringlesModel_readDouble(pars);		
 ...
 */
static PyObject * calculate_ER(CPringlesModel *self) {
 ...
 */
static PyObject * calculate_VR(CPringlesModel *self) {
 ...
 * @return: function value
 */
static PyObject * runXY(CPringlesModel *self, PyObject *args) {
	...
	if( PyList_Check(pars)==1) {
	    ...
	    // to evaluate I(qx,qy)
	    qx_value = CPringlesModel_readDouble(PyList_GET_ITEM(pars,0));
	    qy_value = CPringlesModel_readDouble(PyList_GET_ITEM(pars,1));
	    return Py_BuildValue("d",(*(self->model))(qx_value,qy_value));

	} else {

		// We have a scalar q, we will evaluate I(q)
		qx_value = CPringlesModel_readDouble(pars);		
		
		return Py_BuildValue("d",(*(self->model))(qx_value));
	}	
}

static PyObject * reset(CPringlesModel *self, PyObject *args) {
    

    return Py_BuildValue("d",0.0);
}

static PyObject * set_dispersion(CPringlesModel *self, PyObject *args) {
	PyObject * disp;
	const char * par_name;

	if ( !PyArg_ParseTuple(args,"sO", &par_name, &disp) ) {
	    PyErr_SetString(CPringlesModelError,
	    	"CPringlesModel.set_dispersion expects a DispersionModel object.");
		return NULL;
	}
    ...
    } else    if (!strcmp(par_name, "alpha")) {
        self->model->alpha.dispersion = dispersion;
    } else    if (!strcmp(par_name, "beta")) {
        self->model->beta.dispersion = dispersion;
    } else {
	    PyErr_SetString(CPringlesModelError,
	    	"CPringlesModel.set_dispersion expects a valid parameter name.");
		return NULL;
	}
...
static PyMethodDef CPringlesModel_methods[] = {
...
static PyTypeObject CPringlesModelType = {
    PyObject_HEAD_INIT(NULL)
    0,                         /*ob_size*/
    "CPringlesModel",             /*tp_name*/
    sizeof(CPringlesModel),             /*tp_basicsize*/
    0,                         /*tp_itemsize*/
    (destructor)CPringlesModel_dealloc, /*tp_dealloc*/
    0,                         /*tp_print*/
    0,                         /*tp_getattr*/
    0,                         /*tp_setattr*/
    0,                         /*tp_compare*/
    ...
    0,		               /* tp_iter */
    0,		               /* tp_iternext */
    CPringlesModel_methods,             /* tp_methods */
    CPringlesModel_members,             /* tp_members */
    0,                         /* tp_getset */
    0,                         /* tp_base */
    0,                         /* tp_dict */
    0,                         /* tp_descr_get */
    0,                         /* tp_descr_set */
    0,                         /* tp_dictoffset */
    (initproc)CPringlesModel_init,      /* tp_init */
    0,                         /* tp_alloc */
    CPringlesModel_new,                 /* tp_new */
};
...
void addCPringlesModel(PyObject *module) {
    ...
    if (PyType_Ready(&CPringlesModelType) < 0)
        return;

    Py_INCREF(&CPringlesModelType);
    PyModule_AddObject(module, "CPringlesModel", (PyObject *)&CPringlesModelType);
    
    d = PyModule_GetDict(module);
    static char error_name[] = "CPringlesModel.error";
    CPringlesModelError = PyErr_NewException(error_name, NULL, NULL);
    PyDict_SetItemString(d, "CPringlesModelError", CPringlesModelError);
}
