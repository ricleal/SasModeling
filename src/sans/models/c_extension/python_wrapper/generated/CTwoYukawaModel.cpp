 ...
 *          DO NOT MODIFY THIS FILE, MODIFY src/sans/models/include/TwoYukawa.h
...
static PyObject * CTwoYukawaModelError = NULL;
...
static void
CTwoYukawaModel_dealloc(CTwoYukawaModel* self)
{
...
static PyObject *
CTwoYukawaModel_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
    CTwoYukawaModel *self;
    
    self = (CTwoYukawaModel *)type->tp_alloc(type, 0);
   
    return (PyObject *)self;
}

static int
CTwoYukawaModel_init(CTwoYukawaModel *self, PyObject *args, PyObject *kwds)
{
    if (self != NULL) {
	...
	self->model = new TwoYukawaModel();

        // Initialize parameter dictionary
        PyDict_SetItemString(self->params,"decayConst_Z1",Py_BuildValue("d",10.000000000000));
        PyDict_SetItemString(self->params,"decayConst_Z2",Py_BuildValue("d",2.000000000000));
        ...
        PyDict_SetItemString(self->params,"scale_K1",Py_BuildValue("d",6.000000000000));
        PyDict_SetItemString(self->params,"scale_K2",Py_BuildValue("d",-1.000000000000));
...
static PyMemberDef CTwoYukawaModel_members[] = {
    {name_params, T_OBJECT, offsetof(CTwoYukawaModel, params), 0, def_params},
	{name_dispersion, T_OBJECT, offsetof(CTwoYukawaModel, dispersion), 0, def_dispersion},     
    {name_log, T_OBJECT, offsetof(CTwoYukawaModel, log), 0, def_log},
    {NULL}  /* Sentinel */
};
...
/** Read double from PyObject
    ...
    @return double
*/
double CTwoYukawaModel_readDouble(PyObject *p) {
...
static PyObject *evaluateOneDim(TwoYukawaModel* model, PyArrayObject *q){
 ...
 * @return: numpy array object 
 */
 static PyObject * evaluateTwoDimXY( TwoYukawaModel* model, 
...
#pragma omp parallel for
        ...
        return PyArray_Return(result); 
		    ...
		    PyErr_SetString(CTwoYukawaModelError, 
                   "CTwoYukawaModel.evaluateTwoDimXY couldn't run.");
	        return NULL;
		}      	
}
/**
 ...
 *
 */ 
static PyObject * evalDistribution(CTwoYukawaModel *self, PyObject *args){
	PyObject *qx, *qy;
	PyArrayObject * pars;
	int npars ,mpars;
    ...
    self->model->decayConst_Z1 = PyFloat_AsDouble( PyDict_GetItemString(self->params, "decayConst_Z1") );
    self->model->decayConst_Z2 = PyFloat_AsDouble( PyDict_GetItemString(self->params, "decayConst_Z2") );
    ...
    self->model->scale_K1 = PyFloat_AsDouble( PyDict_GetItemString(self->params, "scale_K1") );
    self->model->scale_K2 = PyFloat_AsDouble( PyDict_GetItemString(self->params, "scale_K2") );

	
	// Get input and determine whether we have to supply a 1D or 2D return value.
	if ( !PyArg_ParseTuple(args,"O",&pars) ) {
	    PyErr_SetString(CTwoYukawaModelError, 
	    	"CTwoYukawaModel.evalDistribution expects a q value.");
		return NULL;
	}
    // Check params
    ...
    if(PyArray_Check(pars)==1) {
	    ...
	    if(npars==1) {
		    ...
		    PyErr_SetString(CTwoYukawaModelError, 
                   "CTwoYukawaModel.evalDistribution expect numpy array of one dimension.");
	        return NULL;
		}
	    ...
	    if(mpars!=2) {
	    	PyErr_SetString(CTwoYukawaModelError, 
	    		"CTwoYukawaModel.evalDistribution expects a list of dimension 2.");
	    	return NULL;
	    }
	     ...
	     if (PyArray_Check(qx) && PyArray_Check(qy)) {
		    ...
		    PyErr_SetString(CTwoYukawaModelError, 
                   "CTwoYukawaModel.evalDistribution expect 2 numpy arrays in list.");
	        return NULL;
	     }
	}
	PyErr_SetString(CTwoYukawaModelError, 
                   "CTwoYukawaModel.evalDistribution couln't be run.");
	return NULL;
 ...
 */
static PyObject * run(CTwoYukawaModel *self, PyObject *args) {
	...
	if ( !PyArg_ParseTuple(args,"O",&pars) ) {
	    PyErr_SetString(CTwoYukawaModelError, 
	    	"CTwoYukawaModel.run expects a q value.");
		return NULL;
	}
	...
	if( PyList_Check(pars)==1) {
	    ...
	    if(npars!=2) {
	    	PyErr_SetString(CTwoYukawaModelError, 
	    		"CTwoYukawaModel.run expects a double or a list of dimension 2.");
	    	return NULL;
	    }
	    // We have a vector q, get the q and phi values at which
	    // to evaluate I(q,phi)
	    q_value = CTwoYukawaModel_readDouble(PyList_GET_ITEM(pars,0));
	    phi_value = CTwoYukawaModel_readDouble(PyList_GET_ITEM(pars,1));
	    // Skip zero
	    if (q_value==0) {
		...
		q_value = CTwoYukawaModel_readDouble(pars);		
 ...
 */
static PyObject * calculate_ER(CTwoYukawaModel *self) {
 ...
 */
static PyObject * calculate_VR(CTwoYukawaModel *self) {
 ...
 * @return: function value
 */
static PyObject * runXY(CTwoYukawaModel *self, PyObject *args) {
	...
	if( PyList_Check(pars)==1) {
	    ...
	    // to evaluate I(qx,qy)
	    qx_value = CTwoYukawaModel_readDouble(PyList_GET_ITEM(pars,0));
	    qy_value = CTwoYukawaModel_readDouble(PyList_GET_ITEM(pars,1));
	    return Py_BuildValue("d",(*(self->model))(qx_value,qy_value));

	} else {

		// We have a scalar q, we will evaluate I(q)
		qx_value = CTwoYukawaModel_readDouble(pars);		
		
		return Py_BuildValue("d",(*(self->model))(qx_value));
	}	
}

static PyObject * reset(CTwoYukawaModel *self, PyObject *args) {
    

    return Py_BuildValue("d",0.0);
}

static PyObject * set_dispersion(CTwoYukawaModel *self, PyObject *args) {
	PyObject * disp;
	const char * par_name;

	if ( !PyArg_ParseTuple(args,"sO", &par_name, &disp) ) {
	    PyErr_SetString(CTwoYukawaModelError,
	    	"CTwoYukawaModel.set_dispersion expects a DispersionModel object.");
		return NULL;
	}
 ...
 {
	    PyErr_SetString(CTwoYukawaModelError,
	    	"CTwoYukawaModel.set_dispersion expects a valid parameter name.");
		return NULL;
	}
...
static PyMethodDef CTwoYukawaModel_methods[] = {
...
static PyTypeObject CTwoYukawaModelType = {
    PyObject_HEAD_INIT(NULL)
    0,                         /*ob_size*/
    "CTwoYukawaModel",             /*tp_name*/
    sizeof(CTwoYukawaModel),             /*tp_basicsize*/
    0,                         /*tp_itemsize*/
    (destructor)CTwoYukawaModel_dealloc, /*tp_dealloc*/
    0,                         /*tp_print*/
    0,                         /*tp_getattr*/
    0,                         /*tp_setattr*/
    0,                         /*tp_compare*/
    ...
    "CTwoYukawaModel objects",           /* tp_doc */
    0,		               /* tp_traverse */
    0,		               /* tp_clear */
    0,		               /* tp_richcompare */
    0,		               /* tp_weaklistoffset */
    0,		               /* tp_iter */
    0,		               /* tp_iternext */
    CTwoYukawaModel_methods,             /* tp_methods */
    CTwoYukawaModel_members,             /* tp_members */
    0,                         /* tp_getset */
    0,                         /* tp_base */
    0,                         /* tp_dict */
    0,                         /* tp_descr_get */
    0,                         /* tp_descr_set */
    0,                         /* tp_dictoffset */
    (initproc)CTwoYukawaModel_init,      /* tp_init */
    0,                         /* tp_alloc */
    CTwoYukawaModel_new,                 /* tp_new */
};
...
void addCTwoYukawaModel(PyObject *module) {
    ...
    if (PyType_Ready(&CTwoYukawaModelType) < 0)
        return;

    Py_INCREF(&CTwoYukawaModelType);
    PyModule_AddObject(module, "CTwoYukawaModel", (PyObject *)&CTwoYukawaModelType);
    
    d = PyModule_GetDict(module);
    static char error_name[] = "CTwoYukawaModel.error";
    CTwoYukawaModelError = PyErr_NewException(error_name, NULL, NULL);
    PyDict_SetItemString(d, "CTwoYukawaModelError", CTwoYukawaModelError);
}
