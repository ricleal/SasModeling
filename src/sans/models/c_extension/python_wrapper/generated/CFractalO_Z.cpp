 ...
 *          DO NOT MODIFY THIS FILE, MODIFY src/sans/models/include/FractalQtoN.h
...
static PyObject * CFractalO_ZError = NULL;
...
static void
CFractalO_Z_dealloc(CFractalO_Z* self)
{
...
static PyObject *
CFractalO_Z_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
    CFractalO_Z *self;
    
    self = (CFractalO_Z *)type->tp_alloc(type, 0);
   
    return (PyObject *)self;
}

static int
CFractalO_Z_init(CFractalO_Z *self, PyObject *args, PyObject *kwds)
{
    if (self != NULL) {
	...
	self->model = new FractalO_Z();

        // Initialize parameter dictionary
        PyDict_SetItemString(self->params,"s_fractal",Py_BuildValue("d",2.500000000000));
        PyDict_SetItemString(self->params,"cluster_rg",Py_BuildValue("d",3520.000000000000));
        PyDict_SetItemString(self->params,"scale",Py_BuildValue("d",10000.000000000000));
        PyDict_SetItemString(self->params,"background",Py_BuildValue("d",0.010000000000));
        PyDict_SetItemString(self->params,"m_fractal",Py_BuildValue("d",1.800000000000));
        PyDict_SetItemString(self->params,"primary_rg",Py_BuildValue("d",82.000000000000));
...
static PyMemberDef CFractalO_Z_members[] = {
    {name_params, T_OBJECT, offsetof(CFractalO_Z, params), 0, def_params},
	{name_dispersion, T_OBJECT, offsetof(CFractalO_Z, dispersion), 0, def_dispersion},     
    {name_log, T_OBJECT, offsetof(CFractalO_Z, log), 0, def_log},
    {NULL}  /* Sentinel */
};
...
/** Read double from PyObject
    ...
    @return double
*/
double CFractalO_Z_readDouble(PyObject *p) {
...
static PyObject *evaluateOneDim(FractalO_Z* model, PyArrayObject *q){
 ...
 * @return: numpy array object 
 */
 static PyObject * evaluateTwoDimXY( FractalO_Z* model, 
...
#pragma omp parallel for
        ...
        return PyArray_Return(result); 
		    ...
		    PyErr_SetString(CFractalO_ZError, 
                   "CFractalO_Z.evaluateTwoDimXY couldn't run.");
	        return NULL;
		}      	
}
/**
 ...
 *
 */ 
static PyObject * evalDistribution(CFractalO_Z *self, PyObject *args){
	PyObject *qx, *qy;
	PyArrayObject * pars;
	int npars ,mpars;
    ...
    self->model->s_fractal = PyFloat_AsDouble( PyDict_GetItemString(self->params, "s_fractal") );
    self->model->cluster_rg = PyFloat_AsDouble( PyDict_GetItemString(self->params, "cluster_rg") );
    self->model->scale = PyFloat_AsDouble( PyDict_GetItemString(self->params, "scale") );
    self->model->background = PyFloat_AsDouble( PyDict_GetItemString(self->params, "background") );
    self->model->m_fractal = PyFloat_AsDouble( PyDict_GetItemString(self->params, "m_fractal") );
	...
	if ( !PyArg_ParseTuple(args,"O",&pars) ) {
	    PyErr_SetString(CFractalO_ZError, 
	    	"CFractalO_Z.evalDistribution expects a q value.");
		return NULL;
	}
    // Check params
    ...
    if(PyArray_Check(pars)==1) {
	    ...
	    if(npars==1) {
		    ...
		    PyErr_SetString(CFractalO_ZError, 
                   "CFractalO_Z.evalDistribution expect numpy array of one dimension.");
	        return NULL;
		}
	    ...
	    if(mpars!=2) {
	    	PyErr_SetString(CFractalO_ZError, 
	    		"CFractalO_Z.evalDistribution expects a list of dimension 2.");
	    	return NULL;
	    }
	     ...
	     if (PyArray_Check(qx) && PyArray_Check(qy)) {
		    ...
		    PyErr_SetString(CFractalO_ZError, 
                   "CFractalO_Z.evalDistribution expect 2 numpy arrays in list.");
	        return NULL;
	     }
	}
	PyErr_SetString(CFractalO_ZError, 
                   "CFractalO_Z.evalDistribution couln't be run.");
	return NULL;
 ...
 */
static PyObject * run(CFractalO_Z *self, PyObject *args) {
	...
	if ( !PyArg_ParseTuple(args,"O",&pars) ) {
	    PyErr_SetString(CFractalO_ZError, 
	    	"CFractalO_Z.run expects a q value.");
		return NULL;
	}
	...
	if( PyList_Check(pars)==1) {
	    ...
	    if(npars!=2) {
	    	PyErr_SetString(CFractalO_ZError, 
	    		"CFractalO_Z.run expects a double or a list of dimension 2.");
	    	return NULL;
	    }
	    // We have a vector q, get the q and phi values at which
	    // to evaluate I(q,phi)
	    q_value = CFractalO_Z_readDouble(PyList_GET_ITEM(pars,0));
	    phi_value = CFractalO_Z_readDouble(PyList_GET_ITEM(pars,1));
	    // Skip zero
	    if (q_value==0) {
		...
		q_value = CFractalO_Z_readDouble(pars);		
 ...
 */
static PyObject * calculate_ER(CFractalO_Z *self) {
 ...
 */
static PyObject * calculate_VR(CFractalO_Z *self) {
 ...
 * @return: function value
 */
static PyObject * runXY(CFractalO_Z *self, PyObject *args) {
	...
	if( PyList_Check(pars)==1) {
	    ...
	    // to evaluate I(qx,qy)
	    qx_value = CFractalO_Z_readDouble(PyList_GET_ITEM(pars,0));
	    qy_value = CFractalO_Z_readDouble(PyList_GET_ITEM(pars,1));
	    return Py_BuildValue("d",(*(self->model))(qx_value,qy_value));

	} else {

		// We have a scalar q, we will evaluate I(q)
		qx_value = CFractalO_Z_readDouble(pars);		
		
		return Py_BuildValue("d",(*(self->model))(qx_value));
	}	
}

static PyObject * reset(CFractalO_Z *self, PyObject *args) {
    

    return Py_BuildValue("d",0.0);
}

static PyObject * set_dispersion(CFractalO_Z *self, PyObject *args) {
	PyObject * disp;
	const char * par_name;

	if ( !PyArg_ParseTuple(args,"sO", &par_name, &disp) ) {
	    PyErr_SetString(CFractalO_ZError,
	    	"CFractalO_Z.set_dispersion expects a DispersionModel object.");
		return NULL;
	}
 ...
 {
	    PyErr_SetString(CFractalO_ZError,
	    	"CFractalO_Z.set_dispersion expects a valid parameter name.");
		return NULL;
	}
...
static PyMethodDef CFractalO_Z_methods[] = {
...
static PyTypeObject CFractalO_ZType = {
    PyObject_HEAD_INIT(NULL)
    0,                         /*ob_size*/
    "CFractalO_Z",             /*tp_name*/
    sizeof(CFractalO_Z),             /*tp_basicsize*/
    0,                         /*tp_itemsize*/
    (destructor)CFractalO_Z_dealloc, /*tp_dealloc*/
    0,                         /*tp_print*/
    0,                         /*tp_getattr*/
    0,                         /*tp_setattr*/
    0,                         /*tp_compare*/
    ...
    0,		               /* tp_iter */
    0,		               /* tp_iternext */
    CFractalO_Z_methods,             /* tp_methods */
    CFractalO_Z_members,             /* tp_members */
    0,                         /* tp_getset */
    0,                         /* tp_base */
    0,                         /* tp_dict */
    0,                         /* tp_descr_get */
    0,                         /* tp_descr_set */
    0,                         /* tp_dictoffset */
    (initproc)CFractalO_Z_init,      /* tp_init */
    0,                         /* tp_alloc */
    CFractalO_Z_new,                 /* tp_new */
};
...
void addCFractalO_Z(PyObject *module) {
    ...
    if (PyType_Ready(&CFractalO_ZType) < 0)
        return;

    Py_INCREF(&CFractalO_ZType);
    PyModule_AddObject(module, "CFractalO_Z", (PyObject *)&CFractalO_ZType);
    
    d = PyModule_GetDict(module);
    static char error_name[] = "CFractalO_Z.error";
    CFractalO_ZError = PyErr_NewException(error_name, NULL, NULL);
    PyDict_SetItemString(d, "CFractalO_ZError", CFractalO_ZError);
}
