 ...
 *          DO NOT MODIFY THIS FILE, MODIFY src/sans/models/include/dabmodel.h
...
static PyObject * CDABModelError = NULL;
...
static void
CDABModel_dealloc(CDABModel* self)
{
...
static PyObject *
CDABModel_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
    CDABModel *self;
    
    self = (CDABModel *)type->tp_alloc(type, 0);
   
    return (PyObject *)self;
}

static int
CDABModel_init(CDABModel *self, PyObject *args, PyObject *kwds)
{
    if (self != NULL) {
	...
	self->model = new DABModel();

        // Initialize parameter dictionary
        PyDict_SetItemString(self->params,"length",Py_BuildValue("d",50.000000000000));
...
static PyMemberDef CDABModel_members[] = {
    {name_params, T_OBJECT, offsetof(CDABModel, params), 0, def_params},
	{name_dispersion, T_OBJECT, offsetof(CDABModel, dispersion), 0, def_dispersion},     
    {name_log, T_OBJECT, offsetof(CDABModel, log), 0, def_log},
    {NULL}  /* Sentinel */
};
...
/** Read double from PyObject
    ...
    @return double
*/
double CDABModel_readDouble(PyObject *p) {
...
static PyObject *evaluateOneDim(DABModel* model, PyArrayObject *q){
 ...
 * @return: numpy array object 
 */
 static PyObject * evaluateTwoDimXY( DABModel* model, 
...
#pragma omp parallel for
        ...
        return PyArray_Return(result); 
		    ...
		    PyErr_SetString(CDABModelError, 
                   "CDABModel.evaluateTwoDimXY couldn't run.");
	        return NULL;
		}      	
}
/**
 ...
 *
 */ 
static PyObject * evalDistribution(CDABModel *self, PyObject *args){
	PyObject *qx, *qy;
	PyArrayObject * pars;
	int npars ,mpars;
	...
	if ( !PyArg_ParseTuple(args,"O",&pars) ) {
	    PyErr_SetString(CDABModelError, 
	    	"CDABModel.evalDistribution expects a q value.");
		return NULL;
	}
    // Check params
    ...
    if(PyArray_Check(pars)==1) {
	    ...
	    if(npars==1) {
		    ...
		    PyErr_SetString(CDABModelError, 
                   "CDABModel.evalDistribution expect numpy array of one dimension.");
	        return NULL;
		}
	    ...
	    if(mpars!=2) {
	    	PyErr_SetString(CDABModelError, 
	    		"CDABModel.evalDistribution expects a list of dimension 2.");
	    	return NULL;
	    }
	     ...
	     if (PyArray_Check(qx) && PyArray_Check(qy)) {
		    ...
		    PyErr_SetString(CDABModelError, 
                   "CDABModel.evalDistribution expect 2 numpy arrays in list.");
	        return NULL;
	     }
	}
	PyErr_SetString(CDABModelError, 
                   "CDABModel.evalDistribution couln't be run.");
	return NULL;
 ...
 */
static PyObject * run(CDABModel *self, PyObject *args) {
	...
	if ( !PyArg_ParseTuple(args,"O",&pars) ) {
	    PyErr_SetString(CDABModelError, 
	    	"CDABModel.run expects a q value.");
		return NULL;
	}
	...
	if( PyList_Check(pars)==1) {
	    ...
	    if(npars!=2) {
	    	PyErr_SetString(CDABModelError, 
	    		"CDABModel.run expects a double or a list of dimension 2.");
	    	return NULL;
	    }
	    // We have a vector q, get the q and phi values at which
	    // to evaluate I(q,phi)
	    q_value = CDABModel_readDouble(PyList_GET_ITEM(pars,0));
	    phi_value = CDABModel_readDouble(PyList_GET_ITEM(pars,1));
	    // Skip zero
	    if (q_value==0) {
		...
		q_value = CDABModel_readDouble(pars);		
 ...
 */
static PyObject * calculate_ER(CDABModel *self) {
 ...
 */
static PyObject * calculate_VR(CDABModel *self) {
 ...
 * @return: function value
 */
static PyObject * runXY(CDABModel *self, PyObject *args) {
	...
	if( PyList_Check(pars)==1) {
	    ...
	    // to evaluate I(qx,qy)
	    qx_value = CDABModel_readDouble(PyList_GET_ITEM(pars,0));
	    qy_value = CDABModel_readDouble(PyList_GET_ITEM(pars,1));
	    return Py_BuildValue("d",(*(self->model))(qx_value,qy_value));

	} else {

		// We have a scalar q, we will evaluate I(q)
		qx_value = CDABModel_readDouble(pars);		
		
		return Py_BuildValue("d",(*(self->model))(qx_value));
	}	
}

static PyObject * reset(CDABModel *self, PyObject *args) {
    

    return Py_BuildValue("d",0.0);
}

static PyObject * set_dispersion(CDABModel *self, PyObject *args) {
	PyObject * disp;
	const char * par_name;

	if ( !PyArg_ParseTuple(args,"sO", &par_name, &disp) ) {
	    PyErr_SetString(CDABModelError,
	    	"CDABModel.set_dispersion expects a DispersionModel object.");
		return NULL;
	}
    ...
    if (!strcmp(par_name, "length")) {
	    ...
	    PyErr_SetString(CDABModelError,
	    	"CDABModel.set_dispersion expects a valid parameter name.");
		return NULL;
	}
...
static PyMethodDef CDABModel_methods[] = {
...
static PyTypeObject CDABModelType = {
    PyObject_HEAD_INIT(NULL)
    0,                         /*ob_size*/
    "CDABModel",             /*tp_name*/
    sizeof(CDABModel),             /*tp_basicsize*/
    0,                         /*tp_itemsize*/
    (destructor)CDABModel_dealloc, /*tp_dealloc*/
    0,                         /*tp_print*/
    0,                         /*tp_getattr*/
    0,                         /*tp_setattr*/
    0,                         /*tp_compare*/
    ...
    0,		               /* tp_iter */
    0,		               /* tp_iternext */
    CDABModel_methods,             /* tp_methods */
    CDABModel_members,             /* tp_members */
    0,                         /* tp_getset */
    0,                         /* tp_base */
    0,                         /* tp_dict */
    0,                         /* tp_descr_get */
    0,                         /* tp_descr_set */
    0,                         /* tp_dictoffset */
    (initproc)CDABModel_init,      /* tp_init */
    0,                         /* tp_alloc */
    CDABModel_new,                 /* tp_new */
};
...
void addCDABModel(PyObject *module) {
    ...
    if (PyType_Ready(&CDABModelType) < 0)
        return;

    Py_INCREF(&CDABModelType);
    PyModule_AddObject(module, "CDABModel", (PyObject *)&CDABModelType);
    
    d = PyModule_GetDict(module);
    static char error_name[] = "CDABModel.error";
    CDABModelError = PyErr_NewException(error_name, NULL, NULL);
    PyDict_SetItemString(d, "CDABModelError", CDABModelError);
}
